...
class PoeClient {
  ...
  async initializeDriver() {
    ...
    await this.page.goto("https://poe.com/"); //?, {waitUntil: "networkidle0",});
    await delay(500);
    await this.page.setCookie({ name: "p-b", value: this.poeCookie });
    //?====================================this just for turning off the open in app options
    await delay(500);
    ...
    if (
      (await this.page.$(".LoggedOutBotInfoPage_appButton__DZ5ol")) !== null
    ) {
      console.log(
  ...
  async sendMessage(message) {
    try {
      ...
      while (waitingForMessage) {
        if ((await this.page.$(".Message_botMessageBubble__aYctV")) === null) {
          await delay(5);
          continue;
        }
  ...
  async abortMessage() {
    ...
    if (
      (await this.page.$(".ChatStopMessageButton_stopButton__QOW41")) === null
    ) {
      return false;
    }

    await this.page.locator(".ChatStopMessageButton_stopButton__QOW41").click();
  ...
  async isGenerating(streaming = false) {
    ...
    if (
      (await this.page.$(".ChatMessageActionBar_actionBar__gyeEs")) !== null
    ) {
      return false;
    }
  ...
  async deleteMessages(count) {
    ...
    await this.page.evaluate(() => {
      ...
      if (
        document.querySelectorAll(".Message_errorBubble__Bl92G").length === 0
      ) {
